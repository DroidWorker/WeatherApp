...
class LocalRepositoryImpl(private val weatherDAO: WeatherDao, private val searchDao: SearchDao) :
    LocalRepository {
    ...
    override fun getLastWeather(): Flow<WeatherCurrent> = flow {
        weatherDAO.getLast().collect { weather ->
            weatherDAO.getWeatherDescription(weather.weatherDescriptionId)
                .collect { weatherDescription ->
                    emit(
                        WeatherCurrent(
                            weather.dt,
                            weather.city,
    ...
    override suspend fun saveWeatherFive(weather: WeatherFiveDay) {
        withContext(Dispatchers.IO) {
            ...
            val weatherDescriptionItems: MutableList<com.example.localdb.WeatherDescription> =
                mutableListOf()
            ...
            weatherDAO.insertFiveDaysWeatherList(
                weatherItemList,
                mainDataItems,
                weatherDescriptionItems
            )
        }
    }
    ...
    override fun getWeatherFive(): Flow<WeatherFiveDay> = flow {
        ...
        weatherDAO.getFiveDaysWeather().collect { weatherItems ->
            weatherItems.forEach { item ->
                ...
                weatherItemList.add(
                    com.example.domain.WeatherItem(
                        item.dt,
                        mainData.convertTo(com.example.domain.MainData::class),
                        weatherDescription.convertTo(WeatherDescription::class),
                        item.dt_txt,
                        item.iconUrl
                    )
                )
            }
            if (weatherItemList.isNotEmpty()) emit(
                WeatherFiveDay(
                    weatherItems.firstOrNull()?.city ?: "", weatherItemList
                )
            )
        }
    }
